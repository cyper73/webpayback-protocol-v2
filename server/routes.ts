...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // SERVER-SIDE ROUTE PROTECTION FOR FOUNDER-ONLY PAGES
  // MUST be FIRST to intercept before Vite middleware
  app.get('/automation', (req, res, next) => {
    ...
    if (!isFounderDevice) {
      return res.status(403).send(`
        ...
        <html>
        <head>
          <title>Access Denied - WebPayback Protocol</title>
          <meta charset="UTF-8">
          ...
          <style>
            ...
            h1 { color: #2c3e50; margin-bottom: 15px; font-size: 24px; font-weight: 600; }
            p { color: #5a6c7d; line-height: 1.6; margin-bottom: 20px; }
            .details { 
              background: #f8f9fa; 
              padding: 20px; 
              border-radius: 8px; 
        ...
        <body>
          <div class="container">
            ...
            <p><strong>Automated Pool Manager Access Denied</strong></p>
            <p>This functionality is restricted to the WebPayback Protocol founder with authorized device authentication.</p>
            <div class="details">
              <strong>🛡️ Security Requirements:</strong>
              <ul>
                <li>Authorized device (Windows + Chrome/Firefox)</li>
                <li>Founder-level authentication</li>
                <li>Device fingerprint validation</li>
                <li>Founder wallet: 0x742d35Cc6634C0532925a3b8D7a6d88b86e5f9a8</li>
              </ul>
            </div>
            <div class="footer">
              <span class="brand">WebPayback Protocol</span> - Enterprise Security
            </div>
          </div>
        </body>
        </html>
    ...
    // If authorized, proceed to next middleware (Vite will handle serving the React app)
    next();
  });

  app.get('/allowance-management', (req, res, next) => {
    const userAgent = req.headers['user-agent'] || '';
    const isFounderDevice = userAgent.includes('Windows') && (userAgent.includes('Chrome') || userAgent.includes('Firefox'));
      ...
      return res.status(403).send(`
        ...
        <body>
          <div class="container">
            <div class="icon">🔒</div>
            <h1>Access Restricted</h1>
            <p><strong>Allowance Management Access Denied</strong></p>
            <p>This functionality is restricted to the WebPayback Protocol founder with authorized device authentication.</p>
            <div class="details">
              <strong>🛡️ Security Requirements:</strong>
              <ul>
