...
- **CRITICAL IDOR SECURITY FIX (July 27, 2025)**: Fixed severe IDOR vulnerability in Content Certificate system where external users could view founder's GitHub data. Implemented proper authentication checks in both frontend and backend. Frontend now verifies user authentication and only shows creators belonging to current user (creator.userId === currentUser.id). Backend APIs now validate user session and block unauthorized access attempts with security logging. System no longer defaults to showing founder data to unauthenticated users.
