...
Qloo API Fix: Implemented corrections based on hackathon forum discussion - fixed header case sensitivity ('X-Api-Key' instead of 'X-API-KEY') and updated search endpoint to use correct type parameter 'urn:entity:brand' instead of 'brand'. API now uses proper search endpoint '/search?query=...&type=urn%3Aentity%3Abrand' as specified in forum guidelines (August 4, 2025).
