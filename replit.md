...
- **CRITICAL IDOR SECURITY FIX (July 27, 2025)**: Fixed severe IDOR vulnerability in Content Certificate system where external users could view founder's GitHub data. Implemented device fingerprinting session system that distinguishes between founder's desktop (Windows Chrome = User ID 1 with creator access) and external devices (smartphones = unique User IDs 2-1001 with zero creator access). Frontend verifies user authentication and backend validates device sessions. External devices now see registration portal instead of founder's GitHub data.
